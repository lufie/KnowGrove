...
const ENGLISH: Record<string, string> = {
  ...
  "库外 Markdown 导入目录": "External Markdown import folder",
  "双击库外 Markdown 时先复制到这里，再在 Obsidian 中打开；留空时沿用收集箱路径。原文件不会移动或改写。": "When you double-click Markdown outside the vault, copy it here before opening it in Obsidian. Leave blank to use the inbox. The source file is never moved or changed.",
  "双击用 Obsidian 打开 Markdown": "Open Markdown in Obsidian on double-click",
  "安装 KnowGrove Mac 打开器后，双击 .md 或 .markdown 会把库外文件安全复制到上方目录并立即在当前 Vault 打开；库内文件直接打开，不产生副本。": "Install the KnowGrove Mac opener so double-clicking .md or .markdown safely copies external files into the folder above and opens them in the current vault. Files already in the vault open without a copy.",
  ...
  "已设为默认：库外 Markdown 会复制到上方目录并在当前 Vault 打开；重复打开相同内容不会生成副本，重名但内容不同会安全编号。原文件保持不变。": "Set as default. External Markdown is copied to the folder above and opened in the current vault. Reopening identical content reuses the imported note, while name conflicts get a safe numeric suffix. The source stays unchanged.",
  "重新配置": "Reconfigure",
