...
KnowGrove 源代码仓库已经公开。社区自动审核对 `2.6.1` 给出失败结果；修复版本 `2.7.3` 完成重新扫描并消除了原来的 3 个阻断 Error。Mac 录音交互与停止保存优化版本 `2.8.3` 已正式发布，插件 ID `knowgrove` 已收录到 Obsidian 官方 `community-plugins.json`。当前社区条目仍明确标记为“尚未由 Obsidian 员工人工审核”，不能描述为已经通过人工审核。

已完成：

1. 源代码已合并到公开仓库默认分支 `main`。
2. `2.8.3` 正式标签和 Release 已存在；Manifest、包版本和版本映射均为 `2.8.3`。
3. Release 已包含 `main.js`、`manifest.json` 和 `styles.css`，不包含用户数据或密钥。
4. GitHub 自动发布工作流已执行 Obsidian 官方规则预检、类型检查、229 项测试与生产构建，并为三个 `2.8.3` 发布文件生成构建来源证明。
...
已完成：`2.8.3` 的本地实机验收、GitHub Release、公开资产回装、构建来源证明和官方社区清单更新链路。待完成：真实麦克风停止耗时与 macOS 电话接力恢复的用户手动验收，以及 Obsidian 员工后续人工审核。
