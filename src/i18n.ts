...
const ENGLISH: Record<string, string> = {
  ...
  "新文档按最小属性进入未读列表；只有链接或音频的轻量笔记会继续提取、转录并由 AI 整理。": "New notes enter the unread list with minimal properties. Link-only and audio notes continue through extraction, transcription, and AI organization.",
  ...
  "阅读状态规范": "Reading status convention",
  "固定使用“阅读状态”：缺失代表未读，开始阅读写“在看”，完成后写“已读”。": "Uses the fixed Reading status property: absence means unread, Reading is written when reading begins, and Read when complete.",
  "新笔记阅读状态": "Reading status for new notes",
  "新笔记不会预写“在看”；只有真正开始阅读时才写入阅读状态。": "New notes are not prefilled as Reading. The status is written only when reading actually begins.",
  ...
  "自动初始化新笔记": "Initialize new notes automatically",
  "在跟踪文件夹中新建或导入 Markdown 笔记时，补齐类型、状态和创建时间；不会预写阅读状态。": "When a Markdown note is created or imported in the tracked folder, add type, lifecycle status, and creation date without prefilling reading status.",
  ...
  "在实时阅览或阅读视图中主动滚动、点击，并在文末停留后自动切换为“已读”。实时编辑文字时会暂停，避免误判。": "After active scrolling or clicking in Live Preview or Reading view, mark a note as Read when you remain at the end. Pauses while editing.",
...
const LOCAL_OVERRIDES: Partial<Record<KnowGroveLocale, Record<string, string>>> = {
  ...
  ja: {
    ...
    "最近操作": "最近開いた項目", "最近编辑": "最近編集した項目", "最近新建": "最近作成した項目", "删除多余空行": "余分な空行を削除", "删除选中内容的空行": "選択範囲の空行を削除", "选中内容中没有可删除的空行": "選択範囲に削除できる空行はありません", "已删除选中内容的空行": "選択範囲の空行を削除しました", "AI 图片转文字": "AI画像をテキスト化", "图片转文字": "画像をテキスト化", "图片转文字后台任务": "画像テキスト化のバックグラウンドタスク", "转换本文全部图片": "このノートの画像をすべて変換", "一键移除本文全部图片": "このノートの画像をすべて削除", "开始转换": "変換を開始", "转文字": "テキスト化", "本文没有可转换的图片": "変換できる画像がありません", "本文没有可移除的图片": "削除できる画像がありません", "移除图片引用": "画像参照を削除", "正在启动…": "開始しています…", "图片转文字已结束": "画像のテキスト化が終了しました", "正在转换图片": "画像を変換しています", "取消后续处理": "残りの処理をキャンセル", "正在取消…": "キャンセルしています…", "正在安全停止当前图片…": "現在の画像処理を安全に停止しています…", "正在准备图片": "画像を準備しています", "正在读取图片": "画像を読み込んでいます", "正在调用模型识别": "モデルの応答を待っています", "正在校验识别结果": "認識結果を検証しています", "正在写入图片下方": "画像の下に書き込んでいます", "正在回读验证结果": "保存結果を確認しています", "图片转文字已完成": "画像のテキスト化が完了しました", "图片转文字失败": "画像のテキスト化に失敗しました", "图片转文字已取消": "画像のテキスト化をキャンセルしました", "定位转换位置": "変換位置へ移動", "定位失败，请稍后重试": "変換位置を特定できません。しばらくしてから再試行してください。", "详情": "詳細", "转到后台": "バックグラウンドで続行",
  ...
  ko: {
    ...
    "最近操作": "최근에 연 파일", "最近编辑": "최근에 편집한 파일", "最近新建": "최근에 만든 파일", "删除多余空行": "불필요한 빈 줄 제거", "删除选中内容的空行": "선택 영역의 빈 줄 제거", "选中内容中没有可删除的空行": "선택 영역에 제거할 수 있는 빈 줄이 없습니다", "已删除选中内容的空行": "선택 영역의 빈 줄을 제거했습니다", "AI 图片转文字": "AI 이미지 텍스트 변환", "图片转文字": "이미지 텍스트 변환", "图片转文字后台任务": "백그라운드 이미지 텍스트 변환 작업", "转换本文全部图片": "이 노트의 모든 이미지 변환", "一键移除本文全部图片": "이 노트의 모든 이미지 제거", "开始转换": "변환 시작", "转文字": "텍스트로 변환", "本文没有可转换的图片": "변환할 이미지가 없습니다", "本文没有可移除的图片": "제거할 이미지가 없습니다", "移除图片引用": "이미지 참조 제거", "正在启动…": "시작 중…", "图片转文字已结束": "이미지 텍스트 변환이 종료되었습니다", "正在转换图片": "이미지 변환 중", "取消后续处理": "남은 처리 취소", "正在取消…": "취소 중…", "正在安全停止当前图片…": "현재 이미지 처리를 안전하게 중지하는 중…", "正在准备图片": "이미지 준비 중", "正在读取图片": "이미지 읽는 중", "正在调用模型识别": "모델 응답 대기 중", "正在校验识别结果": "인식 결과 검증 중", "正在写入图片下方": "이미지 아래에 쓰는 중", "正在回读验证结果": "저장 결과 확인 중", "图片转文字已完成": "이미지 텍스트 변환 완료", "图片转文字失败": "이미지 텍스트 변환 실패", "图片转文字已取消": "이미지 텍스트 변환 취소됨", "定位转换位置": "변환 위치로 이동", "定位失败，请稍后重试": "변환 위치를 찾을 수 없습니다. 잠시 후 다시 시도하세요.", "详情": "세부 정보", "转到后台": "백그라운드에서 계속",
  ...
  de: {
    ...
    "最近操作": "Zuletzt geöffnet", "最近编辑": "Zuletzt bearbeitet", "最近新建": "Zuletzt erstellt", "删除多余空行": "Zusätzliche Leerzeilen entfernen", "删除选中内容的空行": "Leerzeilen in der Auswahl entfernen", "选中内容中没有可删除的空行": "Die Auswahl enthält keine entfernbaren Leerzeilen.", "已删除选中内容的空行": "Leerzeilen in der Auswahl wurden entfernt.", "AI 图片转文字": "KI-Bild in Text", "图片转文字": "Bild in Text", "图片转文字后台任务": "Bild-zu-Text-Hintergrundaufgaben", "转换本文全部图片": "Alle Bilder dieser Notiz umwandeln", "一键移除本文全部图片": "Alle Bilder aus dieser Notiz entfernen", "开始转换": "Umwandlung starten", "转文字": "In Text umwandeln", "本文没有可转换的图片": "Diese Notiz enthält keine umwandelbaren Bilder.", "本文没有可移除的图片": "Diese Notiz enthält keine entfernbaren Bilder.", "移除图片引用": "Bildverweise entfernen", "正在启动…": "Wird gestartet…", "图片转文字已结束": "Bild-zu-Text ist beendet", "正在转换图片": "Bilder werden umgewandelt", "取消后续处理": "Verbleibende Verarbeitung abbrechen", "正在取消…": "Wird abgebrochen…", "正在安全停止当前图片…": "Das aktuelle Bild wird sicher angehalten…", "正在准备图片": "Bild wird vorbereitet", "正在读取图片": "Bild wird gelesen", "正在调用模型识别": "Auf das Modell wird gewartet", "正在校验识别结果": "Erkennungsergebnis wird geprüft", "正在写入图片下方": "Text wird unter das Bild geschrieben", "正在回读验证结果": "Gespeichertes Ergebnis wird geprüft", "图片转文字已完成": "Bild-zu-Text abgeschlossen", "图片转文字失败": "Bild-zu-Text fehlgeschlagen", "图片转文字已取消": "Bild-zu-Text abgebrochen", "定位转换位置": "Umwandlungsstelle anzeigen", "定位失败，请稍后重试": "Die Umwandlungsstelle konnte nicht gefunden werden. Versuchen Sie es später erneut.", "详情": "Details", "转到后台": "Im Hintergrund fortsetzen",
  ...
  fr: {
    ...
    "最近操作": "Ouverture récente", "最近编辑": "Modification récente", "最近新建": "Création récente", "删除多余空行": "Supprimer les lignes vides en trop", "删除选中内容的空行": "Supprimer les lignes vides de la sélection", "选中内容中没有可删除的空行": "La sélection ne contient aucune ligne vide à supprimer.", "已删除选中内容的空行": "Les lignes vides de la sélection ont été supprimées.", "AI 图片转文字": "Image IA en texte", "图片转文字": "Image en texte", "图片转文字后台任务": "Tâches d’image en texte en arrière-plan", "转换本文全部图片": "Convertir toutes les images de cette note", "一键移除本文全部图片": "Supprimer toutes les images de cette note", "开始转换": "Démarrer la conversion", "转文字": "Convertir en texte", "本文没有可转换的图片": "Cette note ne contient aucune image convertible.", "本文没有可移除的图片": "Cette note ne contient aucune image à supprimer.", "移除图片引用": "Supprimer les références d’image", "正在启动…": "Démarrage…", "图片转文字已结束": "La conversion d’image en texte est terminée", "正在转换图片": "Conversion des images", "取消后续处理": "Annuler le traitement restant", "正在取消…": "Annulation…", "正在安全停止当前图片…": "Arrêt sécurisé de l’image en cours…", "正在准备图片": "Préparation de l’image", "正在读取图片": "Lecture de l’image", "正在调用模型识别": "Attente du modèle", "正在校验识别结果": "Validation du résultat", "正在写入图片下方": "Écriture sous l’image", "正在回读验证结果": "Vérification du résultat enregistré", "图片转文字已完成": "Conversion d’image en texte terminée", "图片转文字失败": "Échec de la conversion d’image en texte", "图片转文字已取消": "Conversion d’image en texte annulée", "定位转换位置": "Localiser la conversion", "定位失败，请稍后重试": "Impossible de localiser la conversion. Réessayez dans un instant.", "详情": "Détails", "转到后台": "Continuer en arrière-plan",
  ...
  es: {
    ...
    "最近操作": "Abiertos recientemente", "最近编辑": "Editados recientemente", "最近新建": "Creados recientemente", "删除多余空行": "Eliminar líneas vacías adicionales", "删除选中内容的空行": "Eliminar líneas vacías de la selección", "选中内容中没有可删除的空行": "La selección no contiene líneas vacías que se puedan eliminar.", "已删除选中内容的空行": "Se eliminaron las líneas vacías de la selección.", "AI 图片转文字": "Imagen a texto con IA", "图片转文字": "Imagen a texto", "图片转文字后台任务": "Tareas de imagen a texto en segundo plano", "转换本文全部图片": "Convertir todas las imágenes de esta nota", "一键移除本文全部图片": "Eliminar todas las imágenes de esta nota", "开始转换": "Iniciar conversión", "转文字": "Convertir a texto", "本文没有可转换的图片": "Esta nota no contiene imágenes convertibles.", "本文没有可移除的图片": "Esta nota no contiene imágenes para eliminar.", "移除图片引用": "Eliminar referencias de imagen", "正在启动…": "Iniciando…", "图片转文字已结束": "La conversión de imagen a texto ha terminado", "正在转换图片": "Convirtiendo imágenes", "取消后续处理": "Cancelar el procesamiento restante", "正在取消…": "Cancelando…", "正在安全停止当前图片…": "Deteniendo de forma segura la imagen actual…", "正在准备图片": "Preparando imagen", "正在读取图片": "Leyendo imagen", "正在调用模型识别": "Esperando al modelo", "正在校验识别结果": "Validando el resultado", "正在写入图片下方": "Escribiendo debajo de la imagen", "正在回读验证结果": "Verificando el resultado guardado", "图片转文字已完成": "Conversión de imagen a texto completada", "图片转文字失败": "Error en la conversión de imagen a texto", "图片转文字已取消": "Conversión de imagen a texto cancelada", "定位转换位置": "Localizar conversión", "定位失败，请稍后重试": "No se pudo localizar la conversión. Vuelve a intentarlo en unos instantes.", "详情": "Detalles", "转到后台": "Continuar en segundo plano",
  ...
  "pt-BR": {
    ...
    "最近操作": "Abertos recentemente", "最近编辑": "Editados recentemente", "最近新建": "Criados recentemente", "删除多余空行": "Remover linhas em branco extras", "删除选中内容的空行": "Remover linhas em branco da seleção", "选中内容中没有可删除的空行": "A seleção não contém linhas em branco que possam ser removidas.", "已删除选中内容的空行": "As linhas em branco da seleção foram removidas.", "AI 图片转文字": "Imagem em texto com IA", "图片转文字": "Imagem em texto", "图片转文字后台任务": "Tarefas de imagem em texto em segundo plano", "转换本文全部图片": "Converter todas as imagens desta nota", "一键移除本文全部图片": "Remover todas as imagens desta nota", "开始转换": "Iniciar conversão", "转文字": "Converter em texto", "本文没有可转换的图片": "Esta nota não contém imagens conversíveis.", "本文没有可移除的图片": "Esta nota não contém imagens para remover.", "移除图片引用": "Remover referências de imagem", "正在启动…": "Iniciando…", "图片转文字已结束": "A conversão de imagem em texto terminou", "正在转换图片": "Convertendo imagens", "取消后续处理": "Cancelar o processamento restante", "正在取消…": "Cancelando…", "正在安全停止当前图片…": "Interrompendo a imagem atual com segurança…", "正在准备图片": "Preparando imagem", "正在读取图片": "Lendo imagem", "正在调用模型识别": "Aguardando o modelo", "正在校验识别结果": "Validando o resultado", "正在写入图片下方": "Gravando abaixo da imagem", "正在回读验证结果": "Verificando o resultado salvo", "图片转文字已完成": "Conversão de imagem em texto concluída", "图片转文字失败": "Falha na conversão de imagem em texto", "图片转文字已取消": "Conversão de imagem em texto cancelada", "定位转换位置": "Localizar conversão", "定位失败，请稍后重试": "Não foi possível localizar a conversão. Tente novamente em instantes.", "详情": "Detalhes", "转到后台": "Continuar em segundo plano",
  ...
  ru: {
    ...
    "最近操作": "Недавно открытые", "最近编辑": "Недавно изменённые", "最近新建": "Недавно созданные", "删除多余空行": "Удалять лишние пустые строки", "删除选中内容的空行": "Удалить пустые строки в выделении", "选中内容中没有可删除的空行": "В выделении нет пустых строк, которые можно удалить.", "已删除选中内容的空行": "Пустые строки в выделении удалены.", "AI 图片转文字": "Изображение в текст с ИИ", "图片转文字": "Изображение в текст", "图片转文字后台任务": "Фоновые задачи преобразования изображений в текст", "转换本文全部图片": "Преобразовать все изображения в заметке", "一键移除本文全部图片": "Удалить все изображения из заметки", "开始转换": "Начать преобразование", "转文字": "Преобразовать в текст", "本文没有可转换的图片": "В заметке нет изображений для преобразования.", "本文没有可移除的图片": "В заметке нет изображений для удаления.", "移除图片引用": "Удалить ссылки на изображения", "正在启动…": "Запуск…", "图片转文字已结束": "Преобразование изображения в текст завершено", "正在转换图片": "Преобразование изображений", "取消后续处理": "Отменить оставшуюся обработку", "正在取消…": "Отмена…", "正在安全停止当前图片…": "Безопасная остановка текущего изображения…", "正在准备图片": "Подготовка изображения", "正在读取图片": "Чтение изображения", "正在调用模型识别": "Ожидание модели", "正在校验识别结果": "Проверка результата распознавания", "正在写入图片下方": "Запись под изображением", "正在回读验证结果": "Проверка сохранённого результата", "图片转文字已完成": "Преобразование изображения в текст выполнено", "图片转文字失败": "Ошибка преобразования изображения в текст", "图片转文字已取消": "Преобразование изображения в текст отменено", "定位转换位置": "Перейти к месту преобразования", "定位失败，请稍后重试": "Не удалось найти место преобразования. Повторите попытку позже.", "详情": "Подробнее", "转到后台": "Продолжить в фоновом режиме",
