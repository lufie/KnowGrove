...
- 抖音精选流 `jingxuan?modal_id=` 弹层链接会自动转为标准视频链接，避免下载器返回 `Unsupported URL`。
- 当前页面仍用于单站点授权和 Referer，不扩大站点访问范围。

## 0.3.12

- 当前站点权限未授予时，错误面板改为可操作的「允许当前站点并重试」，不再只刷新弹窗。
- 用户不希望提供登录态时，可直接选择「使用公开解析」。

## 0.3.11

- 点击工具栏入口会自动判断连接状态：未启动时打开 Obsidian 设置，未配对时直接进入确认，已配对时整理当前页面。
- 待确认配对可在再次点击扩展时自动恢复，不再要求用户先进入设置或点击第二个连接按钮。
