...
`2.8.28` 已完成公开 GitHub Release、隔离 clean Vault 回装、当前 Vault 升级回读、社区后台正式扫描和公开目录刷新。`2.8.29` 是本轮待公开发布的新补丁候选：修复“删除选中内容的空行”误删 GFM 表格结构边界的问题，并允许用户再次执行命令时修复被旧版本删除的表格间距；同时同步英文与九种本地化 README 的近期功能和安装说明。只有私有实现合并、公开白名单导出、精确标签和非草稿 GitHub Release、三资产摘要回读、公开地址 clean Vault 首次安装、当前 Vault 升级回读与社区后台接收全部完成后，才能更新为已发布；商店缓存刷新单独记录。
...
2. `2.8.11` 正式标签和 Release 已存在；Manifest、包版本和版本映射均为 `2.8.11`。
