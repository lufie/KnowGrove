...
- 网络边界：公开内容优先匿名读取；受保护媒体只有在用户主动点击、当前站点动态授权后，才把当前 URL 可用的登录态临时交给本机 KnowGrove，不持久化、不上传言续服务器
...
测试语音：`这是言续兼容性测试。每一种音频和视频格式，都应该成功识别这段中文语音。`
...
12 个文件都包含可识别音轨并生成了非空逐字稿。品牌词被 small 模型识别为同音词 `延续`，属于 ASR 准确率问题，不是格式失败。
