...
On macOS, users can explicitly install `~/Applications/KnowGrove Markdown Opener.app` and set it as the default handler for `.md` and `.markdown`. Its configuration stores only the selected vault path, import folder, and previous default app path under `~/Library/Application Support/KnowGrove/`. It reads only Markdown files the operating system sends to it, copies external files into the configured vault without moving or overwriting the source, and opens the imported copy through Obsidian's URI protocol. Users can restore the previous default app from KnowGrove settings.
