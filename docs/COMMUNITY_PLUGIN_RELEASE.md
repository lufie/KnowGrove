...
KnowGrove 源代码仓库已经公开。社区自动审核对 `2.6.1` 给出失败结果；修复版本 `2.7.3` 已正式发布并完成重新扫描，状态为 Completed，原来的 3 个阻断 Error 已全部消失。当前仍应描述为“自动审核完成，待后续人工审核”，不能描述为已经通过全部社区审核。
...
已完成：`2.7.3` 的完整本地验收、GitHub Release、构建来源证明和社区自动复扫。待完成：后续人工审核。
